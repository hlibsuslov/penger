...
const GUIDE_ORDER = [
  ...
  'wallet-comparison', 'opsec', 'passphrase', 'multisig', 'hodl'
];
...
var PAGE_CONFIGS = {
  ...
  '/whitepaper': { view: 'pages/whitepaper', dataKey: 'whitepaper', css: ['about-us.css'], js: [] },
};
...
/* =========================================================
   PROMO CODES — shared module (single source of truth)
   ========================================================= */
const promo = require('./src/promo');
const ALL_CODES = promo.ALL_CODES;
...
app.post('/api/validate-promo', promoLimiter, function (req, res) {
  ...
  var result = promo.validatePromo(code, subtotalCents);
  if (!result.valid) {
    return res.json({ valid: false, error: result.error });
  }

  /* Return discount in EUR (client expects EUR) */
  return res.json({
    valid: true,
    type: result.type,
    value: result.value,
...
const uaOrderLimiter = rateLimit({
  windowMs: 60 * 1000,
  max: 5,
  standardHeaders: true,
  legacyHeaders: false,
...
app.post('/api/ua-order', uaOrderLimiter, function (req, res) {
  var data = req.body;
  if (!data || typeof data !== 'object') return res.status(400).json({ ok: false, error: 'Invalid request' });

  /* ---- S4: Validate required fields ---- */
  if (!data.order_id || typeof data.order_id !== 'string' || data.order_id.length > 50) {
    return res.status(400).json({ ok: false, error: 'Invalid order_id' });
  }
  if (!data.contact || !data.contact.email || !data.contact.firstName) {
    return res.status(400).json({ ok: false, error: 'Missing contact information' });
  }
  var plates = parseInt(data.plates);
  if (!plates || plates < 1 || plates > 4) {
    return res.status(400).json({ ok: false, error: 'Invalid plates count' });
  }
  var validUaMethods = ['ua-cod', 'ua-invoice', 'ua-card'];
  if (!data.pay_method || validUaMethods.indexOf(data.pay_method) === -1) {
    return res.status(400).json({ ok: false, error: 'Invalid payment method' });
  }

  /* ---- C5: Recalculate total server-side ---- */
  var { calcEurTotal } = require('./src/invoice');
  var serverTotalCents = calcEurTotal(data);
  data.value = serverTotalCents / 100;
  data.currency = 'EUR';
  ...
  if (data.promo) promo.incrementUsage(data.promo);
...
/* Promo usage tracking is now handled by src/promo.js (persisted to SQLite) */
