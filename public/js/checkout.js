(function () {
  ...
  contactForm.addEventListener('submit', function (e) {
    ...
    var _dl = window.dataLayer = window.dataLayer || [];
    _dl.push({ event: 'checkout_step', step_name: 'delivery', step_number: 2, page_section: 'checkout' });
    updateMobileCta();
  ...
  if (deliveryForm) {
    deliveryForm.addEventListener('submit', function (e) {
      ...
      var _dl2 = window.dataLayer = window.dataLayer || [];
      _dl2.push({ event: 'checkout_step', step_name: 'payment', step_number: 3, page_section: 'checkout' });
  ...
  payMethods.addEventListener('click', function (e) {
    ...
    var _dl3 = window.dataLayer = window.dataLayer || [];
    _dl3.push({ event: 'payment_method_select', method: payMethod, page_section: 'checkout' });
  ...
  /**
   * Full reset of Solana checkout state → returns UI to payment method selection.
   * Called on: invoice expire, edit contact/delivery, payment failed.
   */
  function resetSolanaCheckout() {
    stopSolanaPolling();
    ...
    /* Hide Solana checkout area + remove stale retry button */
    solanaCheckout.classList.add('hidden');
    var retryBtn = solanaCheckout.querySelector('.solana-retry-btn');
    if (retryBtn) retryBtn.remove();

    /* Restore main checkout controls */
    checkoutBtn.style.display = '';
    checkoutBtn.disabled = !termsCheck.checked;
    if (checkoutBtnText) {
      checkoutBtnText.textContent = countryEl.value === 'UA'
        ? (t.uaOrderBtn || 'Place order')
        : (t.payNow || 'Pay Now');
    }

    var _checkboxesArea = document.getElementById('checkboxesArea');
    if (_checkboxesArea) _checkboxesArea.style.display = '';

    var _trustBadges = stepPayment ? stepPayment.querySelector('.trust-badges') : null;
    if (_trustBadges) _trustBadges.style.display = '';
  }
  ...
  function startCountdown(expiresAt) {
    ...
    function update() {
      ...
      if (remaining <= 0) {
        ...
        resetSolanaCheckout();
        showOrderError(t.cryptoExpiredMsg || 'Quote expired. Please try again.');
      }
    }
    update();
        ...
        fetch('/api/invoice/' + invoiceId)
          ...
          .then(function (data) {
            ...
            if (data.status === 'paid') {
              ...
              try { localStorage.setItem('penger_order', JSON.stringify(orderData)); } catch (e) {}
              ...
              resetSolanaCheckout();
              showOrderError(t.cryptoFailed || 'Payment failed. Please try again.');
            } else {
              doPoll();
            }
          })
          .catch(function () {
            ...
            if (pollFailures >= 10) {
              reconnectAttempts++;
              if (reconnectAttempts <= MAX_RECONNECTS && pollExpiresAt > Date.now()) {
                setSolanaStatus('error', t.cryptoReconnecting || 'Connection lost. Reconnecting...');
                setTimeout(function () {
                  pollFailures = 0;
                  doPoll();
                }, 30000);
  ...
  /* R5: Cross-tab lock to prevent duplicate invoice creation */
  var LOCK_KEY = 'penger_invoice_lock';
  function acquireInvoiceLock(orderId) {
    try {
      var existing = localStorage.getItem(LOCK_KEY);
      if (existing) {
        var lock = JSON.parse(existing);
        if (lock.orderId === orderId && Date.now() - lock.ts < 30000) return false;
      }
      localStorage.setItem(LOCK_KEY, JSON.stringify({ orderId: orderId, ts: Date.now() }));
      return true;
    } catch (e) { return true; }
  }
  ...
  function startSolanaCheckout(asset) {
    ...
    var orderData = collectOrderData();
    if (!acquireInvoiceLock(orderData.order_id)) {
      setSolanaStatus('error', t.cryptoDuplicate || 'Invoice already being created in another tab.');
    ...
    .then(function (data) {
      ...
      /* Re-enable asset buttons + release lock */
      assetBtns.forEach(function (b) { b.disabled = false; });
      releaseInvoiceLock();
