  ...
  function renderDict() {
    ...
    LETTERS.forEach(function (letter) {
      ...
      if (indices.length > WORDS_PER_LETTER) {
        // Hidden container for extra words
        var extra = document.createElement('div');
        extra.className = 'dict-extra-words';
        for (var j = WORDS_PER_LETTER; j < indices.length; j++) {
          buildWordRow(indices[j], extra);
        }
        ...
        moreBtn.innerHTML =
          '<span class="dict-more-text">+' + remaining + ' more</span>';
        (function (btn, extraEl, rem) {
          var expanded = false;
          btn.addEventListener('click', function () {
            ...
            if (expanded) {
              btn.querySelector('.dict-more-text').textContent = 'Show less';
              extraEl.style.maxHeight = extraEl.scrollHeight + 'px';
            } else {
              btn.querySelector('.dict-more-text').textContent = '+' + rem + ' more';
              extraEl.style.maxHeight = '0';
            }
          });
        })(moreBtn, extra, remaining);
