...
function generateInvoicePdf(order, orderId, nbu) {
  ...
  /* ===== TOTALS ===== */
  const totalText = 'Всього до сплати: ' + fmtEur(orderTotal) + ' EUR  ·  В т.ч. у гривнях: ' + fmtUah(orderTotalUah) + ' UAH';
  doc.font('NotoSans').fontSize(9).fill('#1a1a1a');
  doc.text(totalText, 44, doc.y + 4, { width: W, align: 'right' });
  ...
  doc.text('PENGER — BIP39 SELF-CUSTODY TOOLS', 44, 780, { width: W, align: 'center' });
...
function drawPartyBlock(doc, x, y, w, title, rows) {
  ...
  rows.forEach(function (r) {
    ...
    var valH = doc.heightOfString(r[1], { width: w - 68 });
    doc.text(r[1], x + 58, rowY, { width: w - 68 });
    rowY += Math.max(16, valH + 4);
  });
}
