...
/** Payment type badge — distinct emoji + label for each payment method */
function paymentTypeBadge(asset, payMethod) {
  if (asset === 'SOL') return '◎ SOL';
  if (asset === 'USDC') return '💵 USDC';
  if (payMethod) {
    const pm = payMethod.toLowerCase();
    if (pm.includes('cod') || pm.includes('накладен')) return '📦 Накладений платіж';
    if (pm.includes('invoice') || pm.includes('фактур') || pm.includes('рахун')) return '📄 Рахунок';
    if (pm.includes('card') || pm.includes('карт')) return '💳 Карта';
    return '💰 ' + payMethod;
  }
...
function formatInvoiceFull(inv) {
  ...
  const lines = [
    ...
    `💳 Type: ${paymentTypeBadge(inv.asset)}`,
...
function formatInvoiceShort(inv, index) {
  ...
  const badge = paymentTypeBadge(inv.asset);
  return `${index}. ${statusEmoji(inv.status)}\n` +
    `   *${inv.order_id}* — *${fmtEur(inv.amount_eur)}* ${badge}${plates}${country}\n` +
...
function subscribeToEvents() {
  ...
  bus.on('invoice:created', (invoice, orderData) => {
    ...
    const lines = [
      ...
      `💳 ${paymentTypeBadge(invoice.asset)}`,
  ...
  bus.on('order:ua-submitted', (order) => {
    ...
    const lines = [
      ...
      ``,
      `Order: *${order.orderId}*`,
      `💳 ${paymentTypeBadge(null, order.payMethod)}`,
      `Amount: *€${order.total}*`,
    ];
    ...
    lines.push(``);
    lines.push(`*— Delivery —*`);
    if (order.npCity) lines.push(`🏙 ${order.npCity}`);
    if (order.npWarehouse) lines.push(`📦 НП: ${order.npWarehouse}`);
    const addrParts = [addr.street, addr.apt ? 'apt ' + addr.apt : null, addr.zip, addr.city].filter(Boolean);
    if (addrParts.length) lines.push(`📍 ${addrParts.join(', ')}`);

    lines.push(``);
    lines.push(`*— Product —*`);
    lines.push(`🔧 ${order.plates} plate(s) · Sleeves: ${fmtSleeves(order.sleeveColors)}`);
    if (order.punchTool) lines.push(`🔨 Punch tool: Yes`);
    if (order.shipping != null) lines.push(`🚚 Shipping: ${order.shipping > 0 ? '€' + order.shipping : 'Free'}`);
    if (order.discount > 0) lines.push(`🏷 Discount: -€${order.discount}`);
    if (order.promo) lines.push(`🎟 Promo: ${order.promo}`);
    if (order.referral) lines.push(`🤝 Referral: ${order.referral}`);
  ...
  bus.on('invoice:paid', (invoice, txSignature) => {
    ...
    if (invoice.payer_wallet) {
      lines.push(``, `👛 Payer: \`${invoice.payer_wallet}\``);
    }
  ...
  bus.on('invoice:status', (invoice, oldStatus, newStatus) => {
    ...
    const data = parseOrderData(invoice.order_data);
    const name = fullName(data);
    const badge = paymentTypeBadge(invoice.asset);
    ...
    if (newStatus === 'expired') {
      ...
      lines.push(``, `Created: ${fmtDate(invoice.created_at)}`);
      notifyOwner(lines.join('\n'));
      return;
    }
